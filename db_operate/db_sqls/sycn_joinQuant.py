...
def index_stocks():
    ...
    fetch_list_temp = """
        ...
        ;
    """
    db_return = pg_execute(command=JQQuerySQL.securities__w_type_index, returning=True)
...
def index_daily():
    ...
    total_lines_count = 0
    s_return = pg_execute(command=JQQuerySQL.securities__w_type_index, returning=True)
...
@time_cost(logger.info)
def fund_daily():
    logger.info("开始获取 fund_daily 数据...")
    cols = ['code', 'date', 'type', 'open', 'close', 'low', 'high', 'volume', 'money', 'factor', 'high_limit', 'low_limit',
            'avg', 'pre_close', 'paused', 'open_interest']
    conflict_cols = ['code', 'date']
    update_cols = ['type', 'open', 'close', 'low', 'high', 'volume', 'money', 'factor', 'high_limit', 'low_limit', 'avg',
                   'pre_close', 'paused', 'open_interest']
    fields = ['open', 'close', 'low', 'high', 'volume', 'money', 'factor', 'high_limit', 'low_limit', 'avg', 'pre_close', 'paused', 'open_interest']

    total_lines_count = 0
    s_return = pg_execute(command=JQQuerySQL.securities__w_type_funds, returning=True)
    for index_item in s_return:
        # todo: 更新模式，追加未入库的日期。（不能只追加新日期的数据，要注意检查是否有前复权，如果有则需要更新全部日期的数据）
        # 初始化模式，如果库里有则跳过，不更新
        id_return = pg_execute(command=JQQuerySQL.fund_daily__i_code.format(code=index_item["code"]), returning=True)
        if id_return:
            logger.info("[已存在] {} 的行情 ...".format(index_item["code"]))
            continue
        ...
        logger.info("[开始更新] {} 的行情，共计 {} 条 ...".format(index_item["code"], len(lines)))
        pg_insert_on_conflict_batch(table=JQNameSpace.full_table_name("fund_daily"),
                                    cols=cols, lines=lines, conflict_cols=conflict_cols, update_cols=update_cols)
        logger.info("[完成更新] {} 的行情，共计 {} 条 ...".format(index_item["code"], len(lines)))
